...
export function createSwipeSubscription({
  ...
  const touchStarts$ = fromEvent<TouchEvent>(domElement, 'touchstart').pipe(
    map(getTouchCoordinates),
  );
  const touchMoves$ = fromEvent<TouchEvent>(domElement, 'touchmove').pipe(
    map(getTouchCoordinates),
  );
  const touchEnds$ = fromEvent<TouchEvent>(domElement, 'touchend').pipe(
    map(getTouchCoordinates),
  );
